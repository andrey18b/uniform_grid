...
import numpy as np
import argparse
from tqdm import tqdm #Is showing the progress of each loop in the terminal
import time
import os
from torch.optim.lr_scheduler import OneCycleLR #StepLR decays every parameter in a group by multiplicative factor (all step size steps)
#OneCycleLR increases the learning rate to a specific maximum and decreases it to a minimum
from tensorboardX import SummaryWriter #Create GUI interface...

from nn_module.encoder_module import SpatialEncoder2D #Other module spatial encode 2d (encoder)
from nn_module.decoder_module import PointWiseDecoder2DSimple # PointWiseDecode2DSimple (decoder)
...
import torch.nn.functional as F # Contains layers, poolings, activation functions and loss, norm functions
import matplotlib.pyplot as plt # represent the image with matlab
from mpl_toolkits.axes_grid1 import ImageGrid #Place your images created with plt in a grid
import logging # save logging data of your program
import shutil # operation on files, removing, copying and renaming files is important
from einops import rearrange # element reordering on a multidimensional tensor e.g. transpose, reshape, squeeze, unsqueeze, stack, concatenate reduce(ims, 'b h w c -> h w c', 'mean')
# https://einops.rocks/1-einops-basics/
from scipy.io import loadmat # loading matlap file
from torch.utils.data import DataLoader, TensorDataset


# set flags / seeds
torch.backends.cudnn.benchmark = True #Find the best execution algorithm depending on the hardware, not good for changed model
torch.backends.cudnn.deterministic = True # only deterministic convolution algorithms
os.environ["CUDA_LAUNCH_BLOCKING"] = "1" #Adding a new environmental variableC
np.random.seed(1) #setting the see
torch.manual_seed(1) # setting the manual seed
torch.cuda.manual_seed(1) #GPU random number seed
torch.multiprocessing.set_sharing_strategy('file_system') #Multiprocessing setup
torch.autograd.set_detect_anomaly(True) # Running the forward pass with detection enabled will allow the backward pass to print the traceback of the forward operation that created the failing backward function


def build_model(res) -> (SpatialEncoder2D, PointWiseDecoder2DSimple):
    # currently they are hard coded
    encoder = SpatialEncoder2D(
        3,   # a + xy coordinates #input channels
        96,  #  embedding dim of token
        256, # embedding dim of encoded sequence
        4,   # heads
        6,   # depth
        res=res, #resolution
        use_ln=True # use_ln (use layer_normalisation) and embedding dropout 0.05
    )

    decoder = PointWiseDecoder2DSimple(
        256, #latent channels
        1,   # output channels
        scale=0.5,  # output_steps, propagator_depth
        res=res
    )
    #Returns total number of parameters for the parameter in decoder and encoder
    total_params = sum(p.numel() for p in encoder.parameters() if p.requires_grad) + \
                   sum(p.numel() for p in decoder.parameters() if p.requires_grad)
    print(f'Total trainable parameters: {total_params}')
    return encoder, decoder #returns constructed decoder and encoder
...
def get_arguments(parser):
    ...
    parser.add_argument(
        '--resolution', type=int, default=241, help='The interval of when sample snapshots from sequence'
    )
...
if __name__ == '__main__':
    ...
    opt = parser.parse_args() #get the arguments - the passed path to the model
    ...
    ntrain = opt.train_seq_num #Sequences in the training set
    ntest = opt.test_seq_num #Sequences in the test set
    res = opt.resolution # Resolution the snapshots 241

    sub = int((421 - 1) / (res - 1))
    dx = 1./res #stepsize per resolution
    # belowing code is copied from:
    # https://github.com/zongyi-li/fourier_neural_operator/blob/master/fourier_2d.py
    # Data is of the shape (number of samples, grid size, grid size)
    train_data = loadmat(train_data_path)  #loading the matlab file
    ...
    x_train = torch.as_tensor(x_train.reshape(ntrain, res, res, 1), dtype=torch.float32) #Reshape the data to the right structure
    ...
    y_mean = rearrange(y_mean, 'h w c -> (h w) c') #get the average pf one average for each combinattion
